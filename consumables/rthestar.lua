...
SMODS.Consumable {
    ...
    loc_txt = {
        name = 'The star?',
        text = {
            [1] = '{C:attention}Destroy{} 3 random cards',
            [2] = 'and convert 5 random cards',
            [3] = 'to {C:diamonds}Diamods{}'
        }
    },
    cost = 3,
    unlocked = true,
    ...
    for _, playing_card in ipairs(G.hand.cards) do temp_hand[#temp_hand + 1] = playing_card end
        ...
        for i = 1, math.min(8, #temp_hand) do 
        ...
        for i = 1, #affected_cards do
            G.E_MANAGER:add_event(Event({
                ...
                delay = 0.1,
                func = function()
                    assert(SMODS.change_base(affected_cards[i], 'Diamonds'))
                    return true
                end
            }))
        end
