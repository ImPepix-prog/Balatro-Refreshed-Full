...
SMODS.Joker{ --Backrooms
    ...
    loc_txt = {
        ...
        ['text'] = {
            ...
            [2] = 'have a fixed {C:green}1 in 10{} Chance',
    ...
    loc_vars = function(self, info_queue, card)
        ...
        local new_numerator3, new_denominator3 = SMODS.get_probability_vars(card, 1, card.ability.extra.odds3, 'j_Refreshed_backrooms')
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play  then
            if context.other_card:is_face() then
                if SMODS.pseudorandom_probability(card, 'group_0_caec2ded', 1, card.ability.extra.odds, 'j_Refreshed_backrooms', true) then
        ...
        if context.setting_blind  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_d2e9b14c', 1, card.ability.extra.odds, 'j_Refreshed_backrooms', false) then
                    SMODS.calculate_effect({func = function()
                        
                        local created_joker = true
                        G.E_MANAGER:add_event(Event({
                            func = function()
                                local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_Refreshed_stilllife' })
                                if joker_card then
